...
export default function Navbar() {
    ...
    const isActive = (path: string) => {
        return pathname === path ? "text-primary" : "text-white hover:text-primary";
    };
    ...
    return (
        <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? "py-0" : "py-0"}`}>
            {/* Background Element - Handles Blur/Shadow without affecting fixed children stacking context */}
            <div className={`absolute inset-0 transition-all duration-300 -z-10 ${scrolled ? "bg-black/90 backdrop-blur-sm shadow-lg" : "bg-transparent"}`} />
            ...
            <div className="flex items-center justify-between px-4 md:px-8 h-16 md:h-20 w-full">
                <Link href="/" className="z-50 relative h-full shrink-0 flex items-center">
                    <Image
                        ...
                        width={320}
                        height={120}
                        className="h-[80%] w-auto object-contain scale-[1] translate-y-0.6"
                        priority
                    />
                </Link>
                ...
                <div className="hidden md:flex items-center space-x-8">
                    ...
                    <Link href="/quote">
                        <button className="bg-primary hover:bg-primary-hover text-white px-6 py-2 rounded-sm font-semibold transition">
                            Request Quote
                        </button>
                    </Link>
                </div>
            ...
            {/* Mobile Menu Overlay */}
            <div className={`fixed inset-0 bg-black/95 z-40 flex flex-col items-center justify-center space-y-8 transition-transform duration-300 ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"} md:hidden`}>
                ...
                <Link href="/quote" onClick={() => setMobileMenuOpen(false)}>
                    <button className="bg-primary hover:bg-primary-hover text-white px-8 py-3 rounded-sm font-semibold text-xl transition">
                        Request Quote
                    </button>
                </Link>
            </div>
